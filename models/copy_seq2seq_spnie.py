...
class CopyNetSeq2Seq(Model):
    ...
    def forward(self,  # type: ignore
        ...
        # print(source_to_target.size(),target_token_ids.size())
        ...
        if target_tokens:
            ...
            elif self._loss_func == 'cross_entropy':
                set_pred_mask, decoder_ids, dec_out = self._tri_decoder_ce(state, max_decoding_steps)
                output_dict = self._criterion_ce(set_pred_mask, decoder_ids, dec_out, target_tokens["tokens"], target_mask, target_num_mask, max_decoding_steps)
    ...
    def _tri_decoder_ce(self, state, max_decoding_steps):
        bz, msl, hs = state["encoder_hs"].size()
        ngt = self.ngt
        nt = self._decoder_layers
        beam_size = self._beam_size
        ...
        decoder_ids = state["source_to_target"].new_zeros(bz, ngt, max_decoding_steps)
        dec_out = state["encoder_hs"].new_zeros(bz, ngt, max_decoding_steps, self._target_vocab_size)

        state["encoder_hs"] = state["encoder_hs"].unsqueeze(1).repeat(1, ngt, 1, 1).view(bz * ngt, -1, hs) # [bz * ngt, msl, hs]
        state["source_mask"] = state["source_mask"].unsqueeze(1).repeat(1, ngt, 1).view(bz * ngt, -1)
        ...
        #state["decoder_hidden"] = state["set_hs"] # [bz * ngt, hs]
        state["decoder_hidden"] = state["encoder_hs"][:, 0, :].view(bz * ngt, hs) # [bz * ngt * cur_ngt, hs]
        
        state["decoder_hidden_all"] = state["decoder_hidden"].unsqueeze(0).repeat(nt, 1, 1)   # [nt, bz * ngt, hs]
        state["decoder_context"] = state["encoder_hs"].new_zeros(bz * ngt, hs) # [bz * ngt, hs]
        state["decoder_context_all"] = state["encoder_hs"].new_zeros(nt, bz * ngt, hs) # [nt, bz * ngt, hs]
        state["copy_log_probs"] = (state["encoder_hs"].new_zeros(bz * ngt, msl - 2) + 1e-45).log()

        generation_mask = state["encoder_hs"].new_full((bz * ngt, self._target_vocab_size), fill_value = 1.0) # [bz * ngt, vocab_size] 
        copy_mask = state["source_mask"][:, 1:-1].float() # [bz * ngt, msl-2]
        # selective_weights = state["encoder_hs"].new_zeros(copy_mask.size()) # [bz * ngt, msl-2]
        
        dec_ids = state["source_to_target"].new_full((bz * ngt,), fill_value = self._start_index)  # start of triple

        dec_embedded = state["encoder_hs"].new_zeros(bz, ngt, max_decoding_steps, self._target_embedding_dim)
        for t in range(max_decoding_steps):
            dec_inp = self._target_embedder(dec_ids)
            dec_inp = self.dropout(self.LayerNorm(dec_inp))
            expanded_dec_ids = dec_ids.unsqueeze(-1).expand(bz * ngt, msl - 2)
            target_to_source = (state["source_to_target"] == expanded_dec_ids).long()
            selective_weights = util.masked_softmax(state["copy_log_probs"], target_to_source)

            log_probs, state = self._decoder(dec_inp, selective_weights, generation_mask, copy_mask, state)
            generation_log_probs, copy_log_probs = log_probs.split([self._target_vocab_size, msl - 2], dim=-1) # [bz * ngt, vocab_size] [bz * ngt, msl - 2]
            state["copy_log_probs"] = copy_log_probs

            # new_copy_probs = (copy_log_probs.new_zeros(bz * ngt, msl-2, self._target_vocab_size) + 1e-45).log()
            # x_index=torch.linspace(0, bz * ngt-1, steps=bz * ngt).long().unsqueeze(1).repeat(1, msl-2).view(-1).to(state["source_to_target"].device)
            # y_index=torch.linspace(0, msl-3, steps=msl-2).long().repeat(1, bz * ngt).view(-1).to(state["source_to_target"].device)
            # z_index=state["source_to_target"].view(-1)
            # new_copy_probs[x_index,y_index,z_index]=copy_log_probs.contiguous().view(-1)
            # combined_gen_and_copy = torch.cat((generation_log_probs.unsqueeze(1), new_copy_probs), dim=1)
            # final_log_probs = util.logsumexp(combined_gen_and_copy, dim=1)
            
            final_log_probs = self._gather_final_log_probs(generation_log_probs, copy_log_probs, state)[:, :self._target_vocab_size]
            
            _, topi = final_log_probs.topk(1)    # topi: [bz * ngt, 1]
            dec_ids = topi.squeeze(-1)

            dec_embedded[:, :, t, :] = self._target_embedder(dec_ids).view(bz, ngt, -1)
            decoder_ids[:, :, t] = dec_ids.view(bz, ngt)
            dec_out[:, :, t, :] = final_log_probs.view(bz, ngt, -1)

        # for i in range(bz):
        #     print(decoder_ids[i])
        # aaa
        
        target_mask = util.masked_softmax(decoder_ids != self._end_index)
        dec_embedded = util.weighted_sum(dec_embedded, target_mask)
        # [bz, ngt, mtl]
        #target_mask = decoder_ids != self._end_index 
        # [bz, ngt, target_embedding_dim]
        #dec_embedded = (dec_embedded * target_mask.unsqueeze(-1).repeat(1, 1, 1, self._target_embedding_dim)).sum(dim=2)
        #dec_embedded = dec_embedded/(target_mask.sum(-1).unsqueeze(-1) + 1)


        dec_attention_out = self._set_attention(dec_embedded)[0]
        set_pred_mask = self._query_output_layer(dec_attention_out)
        return set_pred_mask, decoder_ids, dec_out


    def _criterion_ce(self, set_pred_mask, decoder_ids, dec_out, target_tokens, target_mask, target_num_mask, max_decoding_steps):
        # set_pred_mask [bz, ngt, 2]
        # target_num_mask [bz, cur_ngt]
        ngt = self.ngt
        bz, cur_ngt = target_num_mask.size()
        gold_target_position = target_num_mask.new_zeros((bz, ngt))

        sim_mat = dec_out.new_zeros((bz, ngt, cur_ngt))

        for i in range(bz):
            for j in range(ngt):
                for k in range(cur_ngt):
                    if int(target_num_mask[i, k]) == 1:
                        num_of_words = int(target_mask[i, k].sum() - 1)
                        sim_mat[i, j, k] = F.nll_loss(dec_out[i, j, :num_of_words], target_tokens[i, k, 1: num_of_words+1])
                    else:
                        sim_mat[i, j, k] = float('inf')
        #print(sim_mat)
        sim_mat_cpu = sim_mat.cpu().detach().numpy()
        ...
        return {"loss": set_loss, "probs": probs}
    ...
    def _forward_beam_search(self, state, set_pred_mask):
        ...
        #generate the set pred mask
        if self._loss_func == 'cross_entropy':
            dec_embedded = self._target_embedder(predictions)
            target_mask = util.masked_softmax(predictions != self._end_index)
            dec_embedded = util.weighted_sum(dec_embedded, target_mask)
            dec_attention_out = self._set_attention(dec_embedded)[0]
            set_pred_mask = self._query_output_layer(dec_attention_out)
    ...
    def take_search_step(self, last_predictions, state):
        ...
        # new_copy_probs = (copy_log_probs.new_zeros(bz, msl-2, self._target_vocab_size) + 1e-45).log()
        # x_index=torch.linspace(0, bz-1, steps=bz).long().unsqueeze(1).repeat(1, msl-2).view(-1).to(state["source_to_target"].device)
        # y_index=torch.linspace(0, msl-3, steps=msl-2).long().repeat(1, bz).view(-1).to(state["source_to_target"].device)
        # z_index=state["source_to_target"].view(-1)
        # new_copy_probs[x_index,y_index,z_index]=copy_log_probs.contiguous().view(-1)
        # combined_gen_and_copy = torch.cat((generation_log_probs.unsqueeze(1), new_copy_probs), dim=1)
        # final_log_probs = util.logsumexp(combined_gen_and_copy, dim=1)
